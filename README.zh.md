...
这是一个面向 `xenoverse.chemverse` 化学环境的 LLM 科学研究智能体。智能体使用
...
.
|-- run.py                 # 运行单次智能体会话
|-- eval.py                # 运行固定的 40 世界评测基准
|-- configs/
|   `-- default.json       # 默认 AgentConfig 配置，包含 LLM API 设置
`-- sci_agent/
    |-- agent.py           # ReAct 推理循环
...
在 `configs/default.json` 中设置 API 凭据：
...
- 总共 40 个预采样世界。
- `easy`、`medium` 各 20 个世界。
- 种子固定：`easy=1000..1019`，`medium=2000..2019`。
- 默认每个世界运行 3 次 trial。
...
无解任务的 baseline 为：easy `50.0`，medium `100.0`。
...
| `--difficulty` | 只运行 `easy` 或 `medium` 世界。 |
...
| `api_key` | `""` | API key，从配置文件读取。 |
| `base_url` | `""` | OpenAI 兼容 API endpoint，从配置文件读取。 |
